...
async function sendActivationEmail(email, fullName, activationLink) {
    try {
        ...
        const mailOptions = {
            from: `"Picklehub.xyz" <${process.env.EMAIL_USER}>`, // Địa chỉ gửi email
            to: email, // Địa chỉ email của người đăng ký
            subject: `Picklehub.xyz đã nhận được đăng ký tài khoản của thành viên ${fullName}`, // Tiêu đề email
            text: `Xin chào ${fullName},\n\Picklehub.xyz đã nhận đăng ký tài khoản của thành viên "${fullName}".\nVui lòng nhấn vào đường link sau hoặc copy nó sang trình duyệt để kích hoạt tài khoản của bạn!\n\n${activationLink}`, // Nội dung email dạng text
        };
...
async function sendRegistrationEmail(managerEmail, clubName, managerName) {
    try {
        ...
        const mailOptions = {
            ...
            subject: `Clubhub đã nhận được thư đăng ký của ${clubName}`, // Tiêu đề email
...
async function sendResetPasswordEmail(email, resetLink) {
    ...
    const mailOptions = {
        ...
        to: email,
        subject: 'Đặt lại mật khẩu của bạn tại picklehub.xyz',
